...
(function () {
  ...
  function mountAd(container) {
    ...
    if (container.closest(".hidden")) return;               // results section not revealed yet
    ...
    // Ask Google for the ad only once the slot actually has a width, otherwise
    // AdSense throws "No slot size for availableWidth=0" and never fills it.
    let tries = 0;
    const requestAd = () => {
      if (container.offsetWidth < 1 && tries++ < 60) { requestAnimationFrame(requestAd); return; }
      try { (window.adsbygoogle = window.adsbygoogle || []).push({}); } catch (e) {}
    };
    requestAnimationFrame(requestAd);
  }
